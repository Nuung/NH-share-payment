...
const { validateUserCardCreate, validateUserCardApprove } = require('../middlewares/validators/userCardValidator');
...
module.exports = (app) => {
  ...
  app.route('/user/card')
    .post(validateUserCardCreate, user_card_api.creatUserCard);

  app.use('/user/card/approve', authMiddleware);
  app.route('/user/card/approve')
    .post(validateUserCardApprove, user_card_api.creatUserCardPayHistory);
